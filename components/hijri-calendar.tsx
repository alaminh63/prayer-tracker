...
export function HijriCalendar() {
  ...
  return (
    <div className="flex flex-col gap-8 w-full mx-auto">
      {/* Header Section */}
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 bg-card/30 backdrop-blur-xl border border-white/10 rounded-3xl p-6 md:p-8">
        ...
        <div className="flex items-center gap-4">
           ...
           {view === "monthly" && (
             <div className="flex items-center gap-2 bg-secondary/30 p-1.5 rounded-2xl border border-white/5">
      ...
      {view === "monthly" ? (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          {/* Left: Monthly Grid */}
          <div className="lg:col-span-8 bg-card/40 backdrop-blur-xl border border-white/10 rounded-4xl overflow-hidden shadow-2xl shadow-black/20">
            ...
            <div className="grid grid-cols-7">
              {paddingDays.map((_, i) => (
                <div key={`padding-${i}`} className="aspect-[1/1.2] md:aspect-square border-r border-b border-white/5 bg-white/[0.01]" />
              ))}
              ...
              {days.map((day, i) => {
                ...
                return (
                  <div 
                    key={i} 
                    className={cn(
                      "aspect-[1/1.2] md:aspect-square border-r border-b border-white/5 p-3 md:p-6 flex flex-col justify-between transition-all relative group overflow-hidden",
                      isToday ? "bg-primary/10" : "hover:bg-white/5",
                      (weekDays.indexOf(day.date.gregorian.weekday.en.substring(0, 3)) === 5) && "bg-emerald-500/5"
                    )}
                  >
          ...
          <div className="lg:col-span-4 space-y-6 lg:sticky lg:top-8">
            <div className="flex items-center justify-between px-2">
              ...
              <div className="h-8 w-8 rounded-full bg-white/5 flex items-center justify-center text-zinc-500">
                <Info size={14} />
              </div>
            </div>
            ...
            <div className="flex flex-col gap-4">
              {days.filter(d => d.date.hijri.holidays.length > 0).length > 0 ? (
                days.filter(d => d.date.hijri.holidays.length > 0).map((day, i) => (
                  <div 
                    key={i} 
                    className="group relative overflow-hidden p-6 rounded-4xl bg-zinc-950/40 border border-white/10 hover:border-primary/30 transition-all duration-500 flex flex-col gap-4"
                  >
                    ...
                    <div className="space-y-2">
                      <h4 className="font-black text-white text-xl leading-tight group-hover:text-primary transition-colors">
                ...
                <div className="p-12 text-center bg-zinc-950/20 rounded-4xl border border-dashed border-white/5 flex flex-col items-center gap-4">
                   <div className="h-12 w-12 rounded-full bg-white/5 flex items-center justify-center text-zinc-600">
                      <Star size={20} className="opacity-20" />
                   </div>
                   <p className="text-sm text-zinc-500 italic font-medium max-w-[200px]">
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      {Object.keys(calendar).sort((a, b) => parseInt(a) - parseInt(b)).map((monthNum) => {
        ...
        return (
          <div key={monthNum} className="p-5 rounded-4xl bg-zinc-950/40 border border-white/5 hover:border-white/10 transition-all flex flex-col gap-4 group">
             <div className="flex items-center justify-between">
               <div className="h-8 w-8 rounded-xl bg-primary/10 flex items-center justify-center text-primary font-black text-sm">
                 {monthNum}
               </div>
               <div className="text-right">
                 <h4 className="text-lg font-black text-white leading-tight">{HIJRI_MONTHS[monthIdx].bn}</h4>
                 <p className="text-[10px] text-zinc-500 uppercase font-bold tracking-widest">{HIJRI_MONTHS[monthIdx].en}</p>
               </div>
             </div>

             <div className="grid grid-cols-7 text-[8px] font-black text-zinc-600 uppercase mb-1">
        ...
        return (
          <div 
            ...
            className={cn(
              ...
              isToday ? "bg-primary text-white font-black" : "text-zinc-400 group-hover:text-zinc-300"
            )}
          >
            {day.date.gregorian.day}
            {hasEvent && !isToday && (
