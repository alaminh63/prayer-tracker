...
export default function HadithPage() {
  ...
  const fetchBookContent = async (bookId: string) => {
    ...
    try {
      ...
      toast.error(t.hadith.load_error)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleShare = (text: string) => {
    if (navigator.share) {
      ...
      toast.success(t.hadith.copy_success)
    }
  }
  ...
  return (
    <AppShell>
      <PageHeader 
        title={<span className="text-gradient">{t.hadith.title}</span>} 
      ...
      <div className="px-4 lg:px-8 pb-20">
        {!selectedBook ? (
          <div className="flex flex-col gap-8  mx-auto">
            ...
            <div className="relative group">
              ...
              <Input 
                ...
                className="pl-14 bg-secondary border-border h-16 rounded-3xl text-lg focus:ring-2 focus:ring-primary/20 transition-all font-medium"
                value={searchQuery}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredBooks.map((book) => (
                ...
                >
                  <Card className="group relative overflow-hidden bg-card border-border p-8 cursor-pointer hover:border-primary/40 transition-all rounded-[2.5rem] shadow-2xl h-full flex flex-col justify-between backdrop-blur-md">
                    ...
                    <div className="relative z-10">
                      ...
                      <h3 className="text-2xl font-black text-foreground mb-2 font-inter">{book.name}</h3>
                      <p className="text-[10px] text-muted-foreground font-black uppercase tracking-[0.3em] mb-8">Authentic Collection</p>
                    </div>
                    ...
                    <div className="relative z-10 mt-auto flex items-center justify-between">
                      <div className="flex items-center text-primary text-sm font-black gap-2 group-hover:gap-4 transition-all uppercase tracking-tighter">
                        {t.hadith.view_book} <ChevronRight className="h-5 w-5" />
                      </div>
                      <Badge variant="outline" className="rounded-full px-3 py-1 bg-secondary border-border text-[10px] text-muted-foreground">
                        Verified
                      </Badge>
                    </div>
                  </Card>
          ...
          <div className="flex flex-col gap-8   mx-auto">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-card/80 border border-border p-6 rounded-[2.5rem] backdrop-blur-2xl sticky top-20 z-40 shadow-2xl">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-xl font-black text-foreground leading-tight">{books.find(b => b.id === selectedBook)?.name}</h2>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />
                    <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                      {t.hadith.page_stat.replace("{current}", currentPage.toString()).replace("{total}", totalPages.toString())}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 bg-secondary p-1.5 rounded-2xl border border-border">
                <Button
                  ...
                  className="rounded-xl h-10 px-4 hover:bg-muted text-xs font-bold"
                >
                  {t.hadith.prev}
                </Button>
            ...
            {loading ? (
              <div className="flex flex-col items-center justify-center py-40 gap-6">
                ...
                <div className="text-center">
                  <p className="text-xl font-bold text-foreground mb-1">{t.hadith.loading}</p>
                  <p className="text-sm text-muted-foreground font-medium italic">{t.hadith.wait}</p>
                </div>
              </div>
            ) : (
              <div className="flex flex-col gap-6">
                  ...
                  >
                    {currentHadiths.map((hadith, index) => (
                      <motion.div
                        ...
                        className="group relative p-10 rounded-[3rem] bg-card border border-border hover:border-primary/30 transition-all shadow-2xl cursor-pointer overflow-hidden backdrop-blur-sm"
                      ...
                      >
                        ...
                        <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8">
                          <div className="flex flex-col gap-1.5">
                            <span className="text-[10px] text-muted-foreground font-black uppercase tracking-[0.2em] mb-1">{t.hadith.hadith_number}</span>
                        ...
                        <div className="relative">
                          <p className="text-xl leading-[1.8] text-foreground font-medium font-inter line-clamp-4 italic group-hover:text-primary transition-colors">
                            "{hadith.text}"
                          </p>
                          <div className="mt-8 flex items-center gap-2 text-primary text-xs font-black uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity">
                            {t.hadith.read_more} <ChevronRight size={14} />
                          </div>
                        </div>
                      </motion.div>
                    ))}
                ...
                <div className="flex items-center justify-center gap-6 pt-16">
                  <Button
                    ...
                    className="rounded-3xl h-14 px-10 border-border hover:bg-primary/10 hover:text-primary transition-all font-black uppercase tracking-widest text-xs"
                  >
                    {t.hadith.prev_page}
                  </Button>
                  <Button
        ...
        {selectedHadithIndex !== null && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.9, y: 20 }}
              className="relative bg-card border border-border w-full max-w-5xl max-h-[90vh] rounded-[3rem] overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.5)] flex flex-col"
              ref={readerRef}
            >
              {/* Reader Header */}
              <div className="flex items-center justify-between p-6 md:p-8 border-b border-border bg-secondary">
                <div className="flex items-center gap-4">
                  <div className="h-12 w-12 bg-primary/10 rounded-2xl flex items-center justify-center text-primary">
                    <BookOpen size={24} />
                  </div>
                  <div>
                    <h3 className="text-xl font-black text-foreground">{books.find(b => b.id === selectedBook)?.name}</h3>
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
                      {t.hadith.hadith_number} #{allHadiths[selectedHadithIndex].hadithnumber}
                    </p>
                  </div>
                </div>
                <Button 
                      ...
                      <div className="flex flex-wrap gap-2 justify-center mb-10">
                        {allHadiths[selectedHadithIndex].grades?.map((g: any, i: number) => (
                          <div key={i} className="flex flex-col items-center gap-1">
                            <span className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest">{g.name || "Status"}</span>
                            <span className="text-xs font-black text-emerald-500 bg-emerald-500/10 px-4 py-1.5 rounded-xl border border-emerald-500/20">{g.grade}</span>
                          </div>
                        ))}
                      </div>

                      <p className="text-2xl md:text-3xl leading-[1.8] text-foreground font-inter text-center font-medium italic">
              ...
              <div className="p-6 md:p-8 border-t border-border bg-secondary flex flex-col md:flex-row items-center justify-between gap-6">
                <div className="flex gap-4">
                  <Button
                    variant="outline"
                    className="h-12 px-6 rounded-2xl border-border hover:bg-muted transition-all text-xs font-bold gap-2"
                    onClick={() => handleShare(allHadiths[selectedHadithIndex].text)}
                  >
                    <Share2 size={16} /> {t.hadith.share}
                  </Button>
                  <Button
                    variant="outline"
                    className="h-12 w-12 rounded-2xl border-border hover:bg-muted transition-all text-muted-foreground"
                    onClick={() => {
                ...
                <div className="flex items-center gap-4">
                  <Button
                    ...
                    onClick={prevHadith}
                    className="h-14 w-14 rounded-full border-border hover:bg-primary/20 hover:text-primary transition-all p-0"
                  >
                    <ChevronLeft size={24} />
                  </Button>
                  <div className="snap-center flex flex-col items-center min-w-[100px]">
                    ...
                    <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest mt-1">
                      {t.hadith.total_kitab.replace("{total}", allHadiths.length.toString())}
                    </span>
                  </div>
                  <Button
                    variant="outline"
